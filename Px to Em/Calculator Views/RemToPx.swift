...
struct EmToPx: View {
    ...
    var body: some View {
        if device == .phone {
            NavigationView {
                EmToPxView(pxResults: self.pxResults, emResults: self.emResults, lhResults: self.lhResults)
            }
        } else {
            EmToPxView(pxResults: self.pxResults, emResults: self.emResults, lhResults: self.lhResults)
        }
...
struct EmToPxView: View {
    ...
    func save(_ calcResult: String) {
        ...
        print("\(String(format: "%.2f", (Double(emTextEmpty) ?? 1.000)))rem is \(String(format: "%.0f", emToPxs(baseInt: Double(baseTextEmpty) ?? 16, emInt: Double(emTextEmpty) ?? 1, scaleInt: Double(scaleTextEmpty) ?? 1)))px at a scale of \(String(format: "%.3f", (Double(scaleTextEmpty) ?? 1))) with a baseline of \(Int(baseTextEmpty) ?? 16)px")
    ...
    var content: some View {
        VStack {
            ...
            VStack (alignment: .center) {
                VStack {
                    Button(action: {
                        save("\(String(format: "%.2f", (Double(emTextEmpty) ?? 1.000)))rem is \(String(format: "%.0f", emToPxs(baseInt: Double(baseTextEmpty) ?? 16, emInt: Double(emTextEmpty) ?? 1, scaleInt: Double(scaleTextEmpty) ?? 1)))px at a scale of \(String(format: "%.3f", (Double(scaleTextEmpty) ?? 1))) with a baseline of \(Int(baseTextEmpty) ?? 16)px")
